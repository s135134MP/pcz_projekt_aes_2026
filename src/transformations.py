import src.helpers as h


S_BOX = (
    ,
)

INV_S_BOX = (
    ,
)

RCON = (
    0x01, 0x02, 0x04, 0x08, 0x10,
    0x20, 0x40, 0x80, 0x1B, 0x36,
)


# Multiply a byte by x in GF(2^8) using the AES irreducible polynomial.
def xtime(a: int) -> int:
    result = (a << 1) & 0xFF
    if a & 0x80:
        result ^= 0x1B
    return result


# Multiply two bytes in GF(2^8) for MixColumns and InvMixColumns.
def gmul(a: int, b: int) -> int:
    result = 0
    for _ in range(8):
        if b & 1:
            result ^= a
        hi = a & 0x80
        a = (a << 1) & 0xFF
        if hi:
            a ^= 0x1B
        b >>= 1
    return result


# Convert one AES block into the project's 4x4 state matrix.
def bytes_to_state(data: bytes) -> list[list[int]]:
    if len(data) != 16:
        raise ValueError("AES block must contain exactly 16 bytes")
    return [[data[row + 4 * column] for column in range(4)] for row in range(4)]


# Convert the 4x4 state matrix back into a byte block.
def state_to_bytes(state: list[list[int]]) -> bytes:
    return bytes(state[row][column] for column in range(4) for row in range(4))


# Print the current AES state in a readable hexadecimal matrix.
def print_state(state: list[list[int]], label: str = "State") -> None:
    print(f"\n{label}:")
    for row in state:
        print("  " + "  ".join(f"{value:02X}" for value in row))


# Rotate each AES state row left by its row index.
def shift_rows(state: list[list[int]]) -> list[list[int]]:
    new_state = [row[:] for row in state]
    for row_index in range(4):
        new_state[row_index] = state[row_index][row_index:] + state[row_index][:row_index]
    return new_state


# Rotate each AES state row right by its row index.
def inv_shift_rows(state: list[list[int]]) -> list[list[int]]:
    new_state = [row[:] for row in state]
    for row_index in range(4):
        new_state[row_index] = (
            state[row_index][-row_index:] + state[row_index][:-row_index]
            if row_index > 0
            else state[row_index][:]
        )
    return new_state


# Apply MixColumns to one AES state column.
def mix_single_column(column: list[int]) -> list[int]:
    s0, s1, s2, s3 = column
    return [
        gmul(0x02, s0) ^ gmul(0x03, s1) ^ s2 ^ s3,
        s0 ^ gmul(0x02, s1) ^ gmul(0x03, s2) ^ s3,
        s0 ^ s1 ^ gmul(0x02, s2) ^ gmul(0x03, s3),
        gmul(0x03, s0) ^ s1 ^ s2 ^ gmul(0x02, s3),
    ]


# Apply MixColumns to every column in the AES state.
def mix_columns(state: list[list[int]]) -> list[list[int]]:
    new_state = [[0] * 4 for _ in range(4)]
    for column_index in range(4):
        column = [state[row][column_index] for row in range(4)]
        mixed = mix_single_column(column)
        for row in range(4):
            new_state[row][column_index] = mixed[row]
    return new_state


# Apply the inverse MixColumns transform to one AES state column.
def inv_mix_single_column(column: list[int]) -> list[int]:
    s0, s1, s2, s3 = column
    return [
        gmul(0x0E, s0) ^ gmul(0x0B, s1) ^ gmul(0x0D, s2) ^ gmul(0x09, s3),
        gmul(0x09, s0) ^ gmul(0x0E, s1) ^ gmul(0x0B, s2) ^ gmul(0x0D, s3),
        gmul(0x0D, s0) ^ gmul(0x09, s1) ^ gmul(0x0E, s2) ^ gmul(0x0B, s3),
        gmul(0x0B, s0) ^ gmul(0x0D, s1) ^ gmul(0x09, s2) ^ gmul(0x0E, s3),
    ]


# Apply the inverse MixColumns transform to every AES state column.
def inv_mix_columns(state: list[list[int]]) -> list[list[int]]:
    new_state = [[0] * 4 for _ in range(4)]
    for column_index in range(4):
        column = [state[row][column_index] for row in range(4)]
        mixed = inv_mix_single_column(column)
        for row in range(4):
            new_state[row][column_index] = mixed[row]
    return new_state


# Convert a round key block into the project state layout.
def round_key_to_state(round_key):
    if len(round_key) != 16:
        raise ValueError("Round key must contain exactly 16 bytes")
    return bytes_to_state(round_key)


# XOR the current state with the selected round key.
def add_round_key(state, round_key):
    round_key_state = round_key_to_state(round_key) if isinstance(round_key, (bytes, bytearray)) else round_key
    for row in range(4):
        for column in range(4):
            state[row][column] ^= round_key_state[row][column]
    return state


# Rotate a 4-byte key schedule word left by one byte.
def rot_word(word):
    return word[1:] + word[:1]


# Substitute every byte in a key schedule word through the S-box.
def sub_word(word):
    return [S_BOX[value] for value in word]


# XOR two 4-byte key schedule words.
def xor_words(left, right):
    return [left[index] ^ right[index] for index in range(4)]


# Substitute every byte in the current AES state through the S-box.
def sub_bytes(state):
    return [[S_BOX[value] for value in row] for row in state]


# Apply the inverse S-box to every byte in the current AES state.
def inv_sub_bytes(state):
    return [[INV_S_BOX[value] for value in row] for row in state]


# Expand a 16-, 24-, or 32-byte AES key into round keys.
def key_schedule(key):
    if not isinstance(key, (bytes, bytearray)):
        raise TypeError("AES key must be bytes-like")

    key = bytes(key)
    if len(key) not in (16, 24, 32):
        raise ValueError("AES key must contain exactly 16, 24, or 32 bytes")

    n_k = len(key) // 4
    n_r = h.get_round_number(key)
    total_words = 4 * (n_r + 1)
    words = [list(key[index:index + 4]) for index in range(0, len(key), 4)]

    while len(words) < total_words:
        temp = words[-1][:]
        word_index = len(words)

        if word_index % n_k == 0:
            temp = sub_word(rot_word(temp))
            temp[0] ^= RCON[(word_index // n_k) - 1]
        elif n_k > 6 and word_index % n_k == 4:
            temp = sub_word(temp)

        words.append(xor_words(words[-n_k], temp))

    return [bytes(sum(words[index:index + 4], [])) for index in range(0, len(words), 4)]
